...
/******************************************************************************
 * FunctionName : uart_flush
 * Description  : flushes UART RX buffer
 * Returns      : NONE
*******************************************************************************/
void ICACHE_FLASH_ATTR
uart_flush(void)
...
/******************************************************************************
 * FunctionName : uart_readtext
 * Description  : Reads text from RX buffer, finishing with '\r'
 * Returns      : text length
*******************************************************************************/
int ICACHE_FLASH_ATTR
uart_readtext(char* buf)
{
	if(UartDev.rcv_buff.BuffState == WRITE_OVER){
		int len = UartDev.rcv_buff.pWritePos - UartDev.rcv_buff.pReadPos;
		os_memcpy(buf, UartDev.rcv_buff.pReadPos, len);
		buf[len] = 0;
		uart_flush();
		return len;
	}
