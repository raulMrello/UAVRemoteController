...
/**********************************************************************************************************
 * \fn tcpclient_sent_cb
 * \brief Data callback. Data has sent, disconnects from remote peer.
 */
...
/**********************************************************************************************************
 * \fn tcpclient_discon_cb
 * \brief Disconnect callback. Reconnection is done every 2 seconds
 */
...
/**********************************************************************************************************
 * \fn tcpclient_connect_cb
 * \brief Connection with remote TCP server has been established. Setup datasent_callback and then sends
 * pending data.
 */
static void ICACHE_FLASH_ATTR tcpclient_connect_cb(void *arg)
{
	...
	/**
	 * Checks if there is pending recv data in UART_RX_BUFFER. If there are pending bytes
	 * then resends through the TCP_link
	 */
	if(uart_readtext(payload) == 0){
...
/**********************************************************************************************************
 * \fn tcpclient_recon_cb
 * \brief Reconnection callback. After reconnection, it checks if needs a new reconnection due to some error,
...
/**********************************************************************************************************
 * \fn platform_reconnect
 * \brief Reconnection function
 */
...
/**********************************************************************************************************
 * \fn senddata
 * \brief Start TCP connection with remote peer. On connection success, callback tcpclient_connect_cb is
 * invoked.
 */
...
/**********************************************************************************************************
 * \fn wifi_check_ip
 * \brief Callback function invoked periodically to check connection status. If connection is established
 * then connects to TCP server to exchange data
 */
...
/**********************************************************************************************************
 * \fn setup_wifi_st_mode
 * \brief Setup Wifi STA mode
 */
void setup_wifi_st_mode(void) {
	...
	if(wifi_station_get_config(&stconfig)) {
		...
		if(!wifi_station_set_config(&stconfig)) {
			#ifdef PLATFORM_DEBUG
			ets_uart_printf("RC_node not set station config!\r\n");
			#endif
		}
	}
	...
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("RC_node in STA mode configured.\r\n");
	#endif
}
 ...
 * 	- Setup UART0 (TX for defug, RX for data reception)
 * 	- Setup Wifi STA mode (connect to QC_wifi)
 * 	- Connect to TCP server on 192.168.5.1:80
 */
void ICACHE_FLASH_ATTR user_init() {
	...
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("Starting RC_node...\r\n");
	#endif
    
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("Setting STA mode...\r\n");
	#endif
	setup_wifi_st_mode();
	...
	#ifdef PLATFORM_DEBUG
	ets_uart_printf("RC_node user_init finished!\r\n");
	#endif
	#ifdef LWIP_DEBUG
	list_lwip_tcp_psc();
	#endif
