...
GPS_NMEA::Errcode GPS_NMEA::Read(uint8_t data){
	char c = (char)data;
    if (c == '$'){                      // NMEA Start
		bufferidx = 0;
        buffer[bufferidx++] = c;
 ...
 ****************************************************************/
// Private Methods //////////////////////////////////////////////////////////////
GPS_NMEA::Errcode  GPS_NMEA::parse_nmea_gps(void){
	uint8_t NMEA_check;
	long aux_deg;
	long aux_min;
	char *parseptr;
...
uint8_t GPS_NMEA::parseHex(char c) {
    if (c < '0')
		return (0);
    if (c <= '9')
		return (c - '0');
...
long GPS_NMEA::parsedecimal(char *str,uint8_t num_car) {
	long d = 0;
	uint8_t i;
	i = num_car;
	while ((str[0] != 0)&&(i>0)) {
...
long GPS_NMEA::parsenumber(char *str, uint8_t numdec) {
	long d = 0;
	uint8_t ndec = 0;
