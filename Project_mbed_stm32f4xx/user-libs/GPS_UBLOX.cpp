/*
	...
	ADAPTED TO WORK ON MBED (by raulMrello)
...
GPS_UBLOX::Errcode GPS_UBLOX::Read(uint8_t data){
	switch(UBX_step){     //Normally we start from zero. This is a state machine
		...
		case 8:			
			...
			if((ck_a==UBX_ck_a)&&(ck_b==UBX_ck_b)){   // Verify the received checksum with the generated checksum.. 
				err = parse_ubx_gps();               // Parse the new GPS packet
			}
			else {
				err = BAD_CRC;
			}
 ...
 ****************************************************************/
// Private Methods //////////////////////////////////////////////////////////////
GPS_UBLOX::Errcode GPS_UBLOX::parse_ubx_gps(){
	int j;
...
long GPS_UBLOX::join_4_bytes(uint8_t Buffer[]){
	union long_union {
		int32_t dword;
		uint8_t  byte[4];
	} longUnion;
