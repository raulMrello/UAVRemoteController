/*************************************************************************//**
 * @file    dev_GUI_State_Formatting.h
 * @mcu		STM32F1x
 * @brief   GUI Device State Machine implementation for Formatting form.
 * @date    15.06.2010
 * @author  Raúl M.
 ****************************************************************************/
...
static void OnFormCreate(void * pThis){
	...
	WndCreate(MENU_OBJ_WINDOW_1,       		// ID
              ...
              (XCHAR *)TEXTS[TXT_Formateando][me->language],	   				// set text 
              me->WindowsScheme);               // default GOL scheme 
   
	StCreate(ID_LABEL_TXT1, 10, 20, GetMaxX()-4, 30, ST_UPDATE | ST_DRAW, (XCHAR *)TEXTS[TXT_Elborradodura][me->language], me->altScheme); 
	StCreate(ID_LABEL_TXT2, 10, 30, GetMaxX()-4, 40, ST_UPDATE | ST_DRAW, (XCHAR *)TEXTS[TXT_Espere_60seg][me->language], me->altScheme); 
}
...
static void OnFormDisplay(void * pThis){
	T_DEV_GUI * me = (T_DEV_GUI*)pThis;	
	FileSystem_Format();
	if(me->OnFormatExit != 0)
		NotifyTransition(me->OnFormatExit);
	else
