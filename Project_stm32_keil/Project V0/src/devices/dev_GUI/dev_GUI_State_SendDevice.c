...
static const uint8_t pduList[] = {PDU_wZONA,PDU_wAJUSTES,PDU_wRELOJ,PDU_wESTACIONES,PDU_wVACACIONES,PDU_wPROGRAMA};
...
static uint8_t StartWrite(void * pThis, char ** pText, uint8_t *pReq, uint8_t *pSzReq, uint8_t **pData, uint8_t *pSzData){
	...
	if(pduList[pduIndex]!= PDU_wPROGRAMA){
		*pText = (char*)TEXTS[TXT_Zona+pduIndex][me->language];
		if(pduList[pduIndex] == PDU_wRELOJ){
			GUI_FormatClockToSend(pThis);
		}
	...
	*pSzData = pduSize[pduIndex];
	do{
		*pData = (uint8_t*)(me->rwPdu->progs + (progIndex * pduSize[pduIndex]));
		progIndex++; // starts with value 0
	}while(**pData == 0 && progIndex < MAX_PROGS);
