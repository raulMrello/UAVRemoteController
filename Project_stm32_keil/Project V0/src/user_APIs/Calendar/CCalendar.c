...
**
**     Description :
**         Compara dos clases de tipo CTime incluyendo el día de la semana
**     Parameters  :
**         CTime *co,*cd : clases a comparar
**     Return :
**         uint8_t : CTime_IGUALES - co.hora = cd.hora
...
uint8_t CTime_CompareEx(CTime *co, CTime *cd){
    ...
    if(co->seg == cd->seg && co->min == cd->min && co->hor == cd->hor && co->sem == cd->sem)
        return(CTime_IGUALES);
    else if( ((co->sem == CTime_DOMINGO) && (cd->sem == CTime_LUNES)) )
        return(CTime_MENOR);
	else if( ((co->sem == CTime_LUNES) && (cd->sem == CTime_DOMINGO)) )
        return(CTime_MAYOR);
	else if(co->sem > cd->sem)
        return(CTime_MENOR);
    else if(co->sem < cd->sem)
        return(CTime_MAYOR);
    else if(co->seg < cd->seg && co->min == cd->min && co->hor == cd->hor)
...
static uint32_t CDate_DaysTillYear(uint8_t year){
	...
	nobisiestos = year-bisiestos;
	dias = (365*nobisiestos)+(366*bisiestos);
	return(dias);
}
...
void 
CCalendar_Config(CCalendar *c, Calendar_cfg_t *cfg){
   	
	CCalendar_Init(c);
	CRtc_Copy(&cfg->summer, &c->objRtc_verano);
	CRtc_Copy(&cfg->winter, &c->objRtc_invierno);
	if(cfg->hemisphere == HEMISPHERE_SOUTH){
		c->status &= ~CCalendar_STAT_NORTE;
	}
	switch(cfg->season_cfg){
		case SEASON_INACTIVE:
			c->status &= ~(CCalendar_STAT_ENEST);
			break;
		case SEASON_MANUAL:
			c->status |= (CCalendar_STAT_ENEST);
			c->status &= ~(CCalendar_STAT_ESTAUTO);
			break;
		case SEASON_AUTO_UE:
			c->status |= (CCalendar_STAT_ENEST + CCalendar_STAT_ESTAUTO);
			break;
	}
...
uint8_t CCalendar_GetFromSeconds(CCalendar *cal, uint32_t counter){
	...
	anios = 4 * cuatrenios;
	diasPorAnio = 366;
	while(diasRestantes >= diasPorAnio){
		anios ++;
		diasRestantes -= diasPorAnio;//TODO RM cambio: diasRestantes -= 365;
		diasPorAnio = 365;
	}
	cal->objRtc_sistema.date.ani = anios; // TODO RM cambio:  + 1;
...
/****************************************************************************************************//**
 * @fun		CCalendar_AddOneMinute
 * @brief	Increments one minute and keep track of the calendar update
 * @param	cal		calendar 
 * @return	
 **********************************************************************************/
void CCalendar_AddOneMinute(CCalendar *cal){
	uint8_t notused;
	if(++cal->objRtc_sistema.time.min > 59){
		cal->objRtc_sistema.time.min = 0;	
		if(++cal->objRtc_sistema.time.hor > 23){
			cal->objRtc_sistema.time.hor = 0;
			cal->objRtc_sistema.date.dia++;
			if(CDate_CheckIntegrity(&cal->objRtc_sistema.date) > 0){
...
/****************************************************************************************************//**
 * @fun		CCalendar_SubOneMinute
 * @brief	Decrements one minute and keep track of the calendar update
 * @param	cal		calendar 
 * @return	
 **********************************************************************************/
void CCalendar_SubOneMinute(CCalendar *cal){
	uint8_t notused;
	if(cal->objRtc_sistema.time.min == 0){
		cal->objRtc_sistema.time.min = 59;	
		if(cal->objRtc_sistema.time.hor == 0){
			cal->objRtc_sistema.time.hor = 23;
			CDate_Decr(&cal->objRtc_sistema.date);
		}
...
void CCalendar_SetFromUTC(CCalendar *cal, CRtc *utc, int16_t utcRef){
	uint8_t notused;
	uint16_t i;
	uint16_t hors = (uint16_t)(utcRef/60);
	uint16_t mins = (uint16_t)(utcRef - (hors*60));
	...
	if(utcRef > 0){
		...
		for(i=0;i<mins;i++)
			CCalendar_AddOneMinute(cal);
	}
	else{
		for(i=0;i<hors;i++)
			CCalendar_SubOneHour(cal);
		for(i=0;i<mins;i++)
			CCalendar_SubOneMinute(cal);
	}
	if(CCalendar_IsSummerTime(cal)){
		CCalendar_AddOneHour(cal);
	}
